...
(async () => {
  ...
  await page.goto("https://doulamatch.net/contact/35440");

  // Capture a screenshot of the captcha image
  const captchaImageElement = await page.$("#captcha-image");
  const captchaImage = await captchaImageElement.screenshot();

  // Use Tesseract to process the captcha image and extract text
  const { data: { text } } = await Tesseract.recognize(captchaImage);
  ...
  // Submit the form
  const submitButton = await page.$(".form-group#submit-button");
