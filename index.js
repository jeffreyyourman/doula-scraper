    ...
    for (let profile of doulaProfilesSet) {
      ...
      try {
        ...
        if (content.includes("<h2>Quota Exceeded</h2>")) {
          ...
          console.log("profileTextNav", profileTextNav); // await page.screenshot({
          ...
          fs.writeFileSync(
            `./dataCollection/dataList/${state}/${profile
              .split("/")
              .pop()}.txt`,
            allProfileTexts
          );
          ...
          if (websites.length > 0) {
            ...
            try {
              ...
              const emailLinks = await page.$$eval(
                'a[href^="mailto:"]',
                (links) =>
                  links.map((link) =>
          ...
          await page.goto(
            `https://doulamatch.net/list/birth/${state}/${pageNum}`,
            {
              waitUntil: "networkidle0",
            }
          );
        ...
        console.error("Doula Match Getting profile error occurred:", error);
